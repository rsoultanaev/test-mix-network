...
def init_mix_network(num_servers, temp_folder, use_existing_config, server_host):
    params = SphinxParams()
    ...
    if not use_existing_config:
        ...
        for i in range(len(private_keys)):
            ...
            mix_nodes.append((server_ids[i], server_host, server_ports[i], public_key_str, private_key_str))
        ...
        for mix_node in mix_nodes:
            output_file.write(','.join(mix_node[:-1]) + '\n')
...
if __name__ == '__main__':
    ...
    arg_parser.add_argument('-a', '--server-host', default='127.0.0.1')
    arg_parser.add_argument('-u', '--use-existing', action='store_true')
    args = arg_parser.parse_args()

    init_mix_network(args.num_servers, args.temp_folder, args.use_existing, args.server_host)
